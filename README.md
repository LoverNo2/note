...
- 🧾 **纯手动保存**：内容只存在内存中，不做任何自动本地存储；点「保存 / 另存为…」（`⌘S`）写入文件后才会持久
- 📄 **保存到本地文件**：顶栏「保存」或 `⌘S` / `Ctrl+S`——首次用系统对话框选择位置并授权，
  之后每次保存**原地覆盖**同一文件，不再反复弹“另存为”；「另存为…」可随时另存一份新备份（Chrome / Edge）
- 📥 **导入与恢复**：支持从备份导入，按 id 合并去重
...
- 笔记数据只存在于**当前页面内存**中：刷新、关闭页面或切换标签会**丢失未保存的内容**。
  请用右上角「保存 / 另存为…」把内容写入本地 JSON 文件（首次选择位置并授权，之后原地覆盖）。
- 应用启动时是空的；要从已有备份继续编辑，点「导入」选择之前保存的 JSON 文件。
...
- 「保存」锁定一个文件后反复原地覆盖；需要另存/留档时用「另存为…」选新位置生成新副本。
  建议定期另存一份或多份历史备份，Safari / Firefox 下保存会自动降级为直接下载。
- 备份文件格式带 `app: "notebook"` 标识与 `version` 字段，导入时会做校验。
